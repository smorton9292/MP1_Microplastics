...
#read in biom table
ASV_reads <- read_biom("18S_Phyto-ASVtable.biom")
...
table(tax_table(BioP.phylo)[, "Super_Groups"], exclude = NULL) #Check
table(tax_table(BioP.phylo)[, "Super_Kingdom"], exclude = NULL) #Check
table(tax_table(BioP.phylo)[, "Kingdom"], exclude = NULL) #20 Bacteria, 779 Unassigned #Remove these two
table(tax_table(BioP.phylo)[, "Phylum"], exclude = NULL) #Check 
table(tax_table(BioP.phylo)[, "Class"], exclude = NULL) #Check 
#table(tax_table(BioP.phylo)[, "Order"], exclude = NULL) #No orders in 18S data to examine
table(tax_table(BioP.phylo)[, "Family"], exclude = NULL) #Check
...
table(tax_table(p0)[, "Domain"], exclude = NULL) #This line checks to see it really was removed 
...
sort(colSums(p6@otu_table),decreasing = FALSE)
...
#Explore data visually before further analysis
...
#Creating stacked bar plot with flipped facet display
...
#Print plot
print(phyplot4) #MANUSCRIPT [Supplementary Figure 7]
...
#Run global PERMANOVAs (Bray-Curtis and Weighted UniFrac at ASV, phyla, and family levels)
set.seed(91)
...
#Since concentrations are in the original phyloseq metadata as 'plastic_conc'
...
#Print updated plots
print(pcoa_plot_elastollan_transparency) #MANUSCRIPT [Supplementary Figure 8]
print(pcoa_plot_tpu181_transparency) #MANUSCRIPT [Supplementary Figure 8]
print(pcoa_plot_tpufc2_transparency) #MANUSCRIPT [Supplementary Figure 8]
...
create_pcoa_plot <- function(phylo_obj, date_label) {
  ...
  #Fit the (chla) variable to the ordination using `envfit`
...
#Biplots for 10 most abundant sig taxa
################ MARCH 7 ################
...
###Process OTU and taxonomy at the family level###
...
###Process OTU and taxonomy at the genus level###
otu <- otu_table(march_23_phylo)
if (taxa_are_rows(otu)) {
  otu <- t(otu)
}
...
#Run envfit at the family level
set.seed(30)
...
add_family_vectors <- function(pcoa_plot, vectors_df) {
  pcoa_plot + 
    ...
    geom_text_repel(
      ...
      min.segment.length = 0.1   #Make sures the segment is drawn
    )
}
...
#Display the updated plots at the family level
print(pcoa_plot_march_7_icon_family)
print(pcoa_plot_march_23_icon_family)
print(pcoa_plot_april_12_icon_family) #MANUSCRIPT [Extended Data Fig. 2]
print(pcoa_plot_may_3_icon_family) #MANUSCRIPT [Extended Data Fig. 2]
print(pcoa_plot_june_9_icon_family) #MANUSCRIPT [Extended Data Fig. 2]
...
#Print updated plots at the genus level
...
#Rerun to these subsets with the non rarefied data for DESeq2 
...
sig_taxa <- as.data.frame(sig_taxa)  #Make sure it is a dataframe
str(sig_taxa)
sig_taxa %>%
...
#Make sure plastic_group exists in sample metadata
...
sig_taxa <- as.data.frame(sig_taxa)  # Ensure it is a dataframe
str(sig_taxa)
sig_taxa %>%
  mutate(Direction = ifelse(log2FoldChange > 0, "Bioplastics", "Elastollan")) %>%
  group_by(Direction) %>%
...
#Check if metadata matches the subset before assigning plastic_group
...
print(may_3_top6_bar_plot) #MANUSCRIPT [Extended Data Fig. 3]
...
print(june_9_top6_bar_plot) #MANUSCRIPT [Extended Data Fig. 3]
...
#Calculate Spearman correlations between families and chla
...
top_algal_drivers <- chla_correlations_long %>%
  ...
  filter(Correlation > 0.5)  #Adjust threshold if needed
...
#Make sure SampleID exists in OTU table
if (!"SampleID" %in% colnames(otu_table_june_9_family)) {
  otu_table_june_9_family <- otu_table_june_9_family %>%
    rownames_to_column(var = "SampleID")
}

#Make sure SampleID formats match
...
#Combine diversity metrics and metadata
meta_alpha <- left_join(alpha_df, meta_df, by = "SampleID")

#Clean up date labels for display
...
#Make sure the date column is in character format
